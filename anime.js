...
/** @enum {Number} */
const tweenTypes = { OBJECT: 0, ATTRIBUTE: 1, CSS: 2, TRANSFORM: 3, CSS_VAR: 4 };

/** @enum {Number} */
const valueTypes = { NUMBER: 0, UNIT: 1, COLOR: 2, COMPLEX: 3 };

/** @enum {Number} */
const tickModes = { NONE: 0, AUTO: 1, FORCE: 2 };

/** @enum {Number} */
const compositionTypes = { replace: 0, none: 1, blend: 2 };
...
const transformsFragmentStrings = /*#__PURE__*/ validTransforms.reduce(
  (a, v) => ({ ...a, [v]: v + "(" }),
  {},
);
...
const isCol = (a) =>
  isHex(a) || ((isRgb(a) || isHsl(a)) && (a[a.length - 1] === ")" || !validRgbHslRgx.test(a)));
/**@param {any} a @return {Boolean} */
...
const parseNumber = (str) =>
  isStr(str) ? parseFloat(/** @type {String} */ (str)) : /** @type {Number} */ (str);
 ...
 * @return {Number}
 */
const snap$1 = (v, increment) =>
  isArr(increment)
    ? increment.reduce((closest, cv) => (abs(cv - v) < abs(closest - v) ? cv : closest))
    : increment
...
const hexToRgba = (hexValue) => {
  ...
  return [
    ...
    hexLength === 5 || hexLength === 9
      ? +(+("0x" + hexValue[isShort ? 4 : 7] + hexValue[isShort ? 4 : 8]) / 255).toFixed(3)
      : 1,
  ];
};
...
const getFunctionValue = (value, target, index, targets, store, prevTween) => {
  ...
  if (isFnc(value)) {
    ...
    func = () => {
      ...
      let computed = getComputedStyle(/** @type {HTMLElement} */ (target))?.getPropertyValue(
        cssVarName,
      );
...
const getCSSValue = (target, propName, animationInlineStyles) => {
  ...
  const value =
    inlineStyles ||
    getComputedStyle(target[proxyTargetSymbol] || target).getPropertyValue(propName);
...
const getOriginalAnimatableValue = (target, propName, tweenType, animationInlineStyles) => {
  ...
  return type === tweenTypes.TRANSFORM
    ? parseInlineTransforms(/** @type {DOMTarget} */ (target), propName, animationInlineStyles)
    : type === tweenTypes.CSS_VAR
      ? getCSSValue(/** @type {DOMTarget} */ (target), propName, animationInlineStyles).trimStart()
...
const composeColorValue = (tween, progress, precision) => {
  ...
  const r = round$1(
    clamp$1(/** @type {Number} */ (mod(lerp$1(fn[0], tn[0], progress))), 0, 255),
    0,
  );
  const g = round$1(
    clamp$1(/** @type {Number} */ (mod(lerp$1(fn[1], tn[1], progress))), 0, 255),
    0,
  );
  const b = round$1(
    clamp$1(/** @type {Number} */ (mod(lerp$1(fn[2], tn[2], progress))), 0, 255),
    0,
  );
  const a = clamp$1(
    /** @type {Number} */ (mod(round$1(lerp$1(fn[3], tn[3], progress), precision))),
    0,
    1,
  );
...
const render = (tickable, time, muteCallbacks, internalRender, tickMode) => {
  ...
  if (
    ...
    if (!_hasChildren) {
      ...
      const forcedRender =
        forcedTick || (isRunningBackwards ? deltaTime * -1 : deltaTime) >= globals.tickThreshold;
      const absoluteTime =
        tickable._offset + (parent ? parent._offset : 0) + tickableDelay + iterationTime;
        ...
        if (
          (forcedRender ||
            ((tweenCurrentTime !== tweenChangeDuration ||
              absoluteTime <= tweenAbsEndTime + (tweenNextRep ? tweenNextRep._delay : 0)) &&
          ...
          (!tweenHasComposition ||
            (!tween._isOverridden &&
              ...
              (!tweenPrevRep ||
                ...
                absoluteTime >=
                  tweenPrevRep._absoluteStartTime + tweenPrevRep._changeDuration + tween._delay)))
        ) {
          const tweenNewTime = (tween._currentTime = clamp$1(
          ...
          const tweenPrecision =
            (tweenIsNumber && tweenIsObject) || tweenProgress === 0 || tweenProgress === 1
              ? -1
              : globals.precision;
          ...
          if (tweenIsNumber) {
            value = number = /** @type {Number} */ (
              tweenModifier(
                round$1(lerp$1(tween._fromNumber, tween._toNumber, tweenProgress), tweenPrecision),
              )
            );
  ...
  if (tickable._hasChildren) {
    ...
    forEachChildren(
      ...
      (/** @type {JSAnimation} */ child) => {
        ...
        const childTickMode =
          child._fps < tl._fps ? child.requestTick(tlCildrenTickTime) : tickMode;
...
const revertValues = (renderable, inlineStylesOnly = false) => {
  ...
  if (renderable._hasChildren) {
    forEachChildren(
      renderable,
      (/** @type {Renderable} */ child) => revertValues(child, inlineStylesOnly),
      true,
    );
  } else {
    ...
    forEachChildren(animation, (/** @type {Tween} */ tween) => {
      ...
      const tweenHadNoInlineValue =
        isNil(originalInlinedValue) || originalInlinedValue === emptyString;
        ...
        if (tweenType === tweenTypes.ATTRIBUTE) {
          if (!inlineStylesOnly) {
            if (tweenHadNoInlineValue) {
              ...
              /** @type {DOMTarget} */ (tweenTarget).setAttribute(
                tweenProperty,
                /** @type {String} */ (originalInlinedValue),
              );
            }
          }
        } else {
...
const additive = { animation: null, update: noop };
...
const getTweenSiblings = (target, property, lookup = "_rep") => {
  ...
  return targetLookup[property]
    ...
    : (targetLookup[property] = { _head: null, _tail: null });
};
  ...
  if (tweenCompositionType === compositionTypes.replace) {
    ...
    if (prevSibling) {
      ...
      if (prevAbsEndTime > absoluteUpdateStartTime) {
        ...
        const updatedPrevChangeDuration = round$1(
...
class Timer extends Clock {
  ...
  constructor(parameters = {}, parent = null, parentPosition = 0) {
    ...
    let timerIterationCount =
      timerLoop === true || timerLoop === Infinity || /** @type {Number} */ (timerLoop) < 0
        ? Infinity
    ...
    this.duration =
      clampInfinity((timerDuration + timerLoopDelay) * timerIterationCount - timerLoopDelay) ||
      minValue;
    /** @type {Boolean} */
  ...
  set currentIteration(iterationCount) {
    this.currentTime =
      this.iterationDuration * clamp$1(+iterationCount, 0, this.iterationCount - 1);
  }
...
const parseEaseString = (string) => {
  ...
  if (string.indexOf("(") <= -1) {
    ...
    const parsedFn = /** @type {EasingFunction} */ (
      hasParams ? /** @type {EasingFunctionWithParams} */ (eases[string])() : eases[string]
    );
...
const generateKeyframes = (keyframes, parameters) => {
  ...
  if (isArr(keyframes)) {
    const propertyNames = []
      .concat(.../** @type {DurationKeyframes} */ (keyframes).map((key) => Object.keys(key)))
      .filter(isKey);
...
class JSAnimation extends Timer {
  ...
  constructor(targets, parameters, parent, parentPosition, fastSet = false, index = 0, allTargets) {
    ...
    const params = /** @type {AnimationParams} */ (
      kfParams
        ? mergeObjects(
            generateKeyframes(/** @type {DurationKeyframes} */ (kfParams), parameters),
            parameters,
          )
        : parameters
    );
      ...
      for (let p in params) {
        if (isKey(p)) {
          ...
          for (let l = keyframes.length; tweenIndex < l; tweenIndex++) {
            ...
            if (!siblings && tweenComposition !== compositionTypes.none)
            ...
            const prevSiblingTween =
              parent && tailTween && tailTween.parent.parent === parent ? tailTween : prevTween;
            ...
            const easeFunctionResult = getFunctionValue(
              easeToParse,
              target,
              ti,
              tl,
            ...
            const keyEasing =
              isFnc(easeFunctionResult) || isStr(easeFunctionResult)
                ? easeFunctionResult
                : easeToParse;
            ...
            const tweenDuration = hasSpring
              ...
              : getFunctionValue(
                  setValue(
                    key.duration,
                    l > 1
                      ? getFunctionValue(tDuration, target, ti, tl, null, prevSiblingTween) / l
                      : tDuration,
                  ),
                  target,
                  ti,
            ...
            if (isFromToValue) {
              ...
              const originalValue = getOriginalAnimatableValue(
                target,
                propName,
                tweenType,
                inlineStylesStore,
              ...
              if (hasToValue) {
                ...
                if (prevTween) {
                  ...
                  decomposeRawValue(
                    parent && prevSibling && prevSibling.parent.parent === parent
                      ? prevSibling._value
            ...
            if (fromTargetObject.t !== toTargetObject.t) {
              if (
                ...
                const complexValue =
                  fromTargetObject.t === valueTypes.COMPLEX ? fromTargetObject : toTargetObject;
                const notComplexValue =
                  fromTargetObject.t === valueTypes.COMPLEX ? toTargetObject : fromTargetObject;
                ...
                const unitValue =
                  fromTargetObject.t === valueTypes.UNIT ? fromTargetObject : toTargetObject;
                const notUnitValue =
                  fromTargetObject.t === valueTypes.UNIT ? toTargetObject : fromTargetObject;
                ...
                const colorValue =
                  fromTargetObject.t === valueTypes.COLOR ? fromTargetObject : toTargetObject;
                const notColorValue =
                  fromTargetObject.t === valueTypes.COLOR ? toTargetObject : fromTargetObject;
            ...
            if (
              ...
              const longestValue =
                fromTargetObject.d.length > toTargetObject.d.length
                  ...
                  : toTargetObject;
              const shortestValue =
                longestValue === fromTargetObject ? toTargetObject : fromTargetObject;
    ...
    this.duration =
      iterationDuration === minValue
        ? minValue
        : clampInfinity(
            (iterationDuration + this._loopDelay) * this.iterationCount - this._loopDelay,
          ) || minValue;
    /** @type {Callback<this>} */
    ...
    forEachChildren(this, (/** @type {Tween} */ tween) => {
      ...
      if (toFunc || fromFunc) {
        if (fromFunc) {
          ...
          decomposeRawValue(
            getOriginalAnimatableValue(tween.target, tween.property, tween._tweenType),
            decomposedOriginalValue,
          );
        ...
        if (toFunc) {
          ...
          tween._toNumber = toTargetObject.o
            ? getRelativeValue(tween._fromNumber, toTargetObject.n, toTargetObject.o)
            : toTargetObject.n;
        }
      }
    });
...
function getTimelineTotalDuration(tl) {
  return (
    clampInfinity((tl.iterationDuration + tl._loopDelay) * tl.iterationCount - tl._loopDelay) ||
    minValue
  );
}
...
function addTlChild(childParams, tl, timePosition, targets, index, allTargets) {
  const isSetter =
    isNum(childParams.duration) && /** @type {Number} */ (childParams.duration) <= minValue;
  ...
  add(a1, a2, a3) {
    ...
    if (isAnim || isTimer) {
      ...
      if (isAnim) {
        ...
        const isStaggerType =
          a3 && /** @type {TweakRegister} */ (a3).type === "Stagger" && globals.editor;
        ...
        if (staggeredPosition || isStaggerType) {
          ...
          const resolvedParams = editorHook
            ? editorHook(/** @type {TargetsParam} */ (a1), childParams, this.id, a3, parsedLength)
            : null;
          // Resolve stagger AFTER editor hook so tweaked position value (a3.defaultValue) is used
          const staggerFn =
            staggeredPosition ||
            globals.editor.resolveStagger(/** @type {TweakRegister} */ (a3).defaultValue);
          parsedTargetsArray.forEach((/** @type {Target} */ target) => {
            ...
            const staggeredTimePosition = parseTimelinePosition(
              this,
              staggerFn(target, i, parsedTargetsArray, null, this),
            );
            addTlChild(
          ...
          const resolvedChildParams = editorHook
            ? editorHook(/** @type {TargetsParam} */ (a1), childParams, this.id, a3)
            : childParams;
          const resolvedPosition =
            a3 && /** @type {*} */ (a3).type ? /** @type {*} */ (a3).defaultValue : a3;
          addTlChild(
            resolvedChildParams,
            this,
  ...
  sync(synced, position) {
    ...
    const duration = +(
      /** @type {globalThis.Animation} */ ((synced).effect
        ? /** @type {globalThis.Animation} */ (synced).effect.getTiming().duration
    ...
    return this.add(
      synced,
      { currentTime: [0, duration], duration, delay: 0, ease: "linear", playbackEase: "linear" },
      position,
    );
  }
...
const createTimeline = (parameters) => {
  if (globals.editor) {
    return /** @type {Timeline} */ (
      /** @type {unknown} */ (globals.editor.addTimeline(parameters))
    );
  }
...
class Animatable {
  ...
  constructor(targets, parameters) {
    ...
    /** @type {AnimationParams} */
    const globalParams = { onBegin: beginHandler, onComplete: pauseHandler, onPause: pauseHandler };
    /** @type {AnimationParams} */
 ...
 * @return {Number}
 */
const mapRange$1 = (value, inLow, inHigh, outLow, outHigh) =>
...
const damp$1 = (start, end, deltaTime, factor) => {
  return !factor
    ...
    : factor === 1
      ? end
      : lerp$1(start, end, 1 - Math.exp(-factor * deltaTime * 0.1));
};
...
class Spring {
  ...
  constructor(parameters = {}) {
    ...
    this.bn = clamp$1(setValue(parameters.bounce, 0.5), -1, 1); // The bounce percentage between -1 and 1.
    this.pd = clamp$1(
      setValue(parameters.duration, 628),
      10 * globals.timeScale,
      maxSpringParamValue * globals.timeScale,
  ...
  calculateBDFromSD() {
    ...
    this.pd = round$1(
      clamp$1(this.pd, 10 * globals.timeScale, maxSpringParamValue * globals.timeScale),
      3,
    );
  }
...
const WAAPIAnimationsLookups = { _head: null, _tail: null };
...
const remove = (targets, renderable, propertyName) => {
  ...
  removeTargetsFromRenderable(targetsArray, /** @type {Renderable} */ (renderable), propertyName);
  return targetsArray;
};
...
class DOMProxy {
  ...
  constructor(el) {
    ...
    this.classList = { add: noop, remove: noop };
  }
  ...
  getBoundingClientRect() {
    return { top: this.y, right: this.x, bottom: this.y + this.height, left: this.x + this.width };
  }
}
...
const parseDraggableFunctionParameter = (value, draggable) =>
  value && isFnc(value) ? /** @type {Function} */ (value)(draggable) : /** @type {T} */ (value);
...
class Draggable {
  ...
  updateBoundingValues() {
    ...
    if (this.contained) {
      ...
      if (this.containerArray) {
        ...
        this.containerBounds[0] = -round$1(
          targetRect.top - (fx ? clamp$1(top, 0, ih) : top) + hiddenTop - cpt,
          0,
        );
        this.containerBounds[1] = -round$1(
          targetRect.right - (fx ? clamp$1(right, 0, iw) : right) - hiddenRight + cpr,
          0,
        );
        this.containerBounds[2] = -round$1(
          targetRect.bottom - (fx ? clamp$1(bottom, 0, ih) : bottom) - hiddenBottom + cpb,
          0,
        );
        this.containerBounds[3] = -round$1(
          targetRect.left - (fx ? clamp$1(left, 0, iw) : left) + hiddenLeft - cpl,
          0,
        );
      }
    }
  ...
  refresh() {
    ...
    const containerPadding = /** @type {[Number, Number, Number, Number]} */ (
    ...
    this.containerFriction = clamp$1(
      setValue(parseDraggableFunctionParameter(params.containerFriction, this), 0.8),
      0,
      1,
    );
    this.releaseContainerFriction = clamp$1(
      setValue(
  ...
  handleMove(e) {
    ...
    if (
      canTouchScroll &&
      ((!isAtTop && !isAtBottom) || (isAtTop && movedY < 0) || (isAtBottom && movedY > 0))
    ) {
      this.pointer[0] = x;
      this.pointer[1] = y;
      this.pointer[2] = x;
      ...
      if (
        this.dragged ||
        (!this.disabled[0] && abs(movedX) > this.dragThreshold) ||
  ...
  handleUp() {
    ...
    if (!disabledX) {
      ...
      springX.velocity =
        disabledY && hasReleaseSpring ? (distanceX ? (ds * directionX) / abs(distanceX) : 0) : pv;
      ...
      durationX =
        cx === dx
          ...
          : hasReleaseSpring
            ? settlingDuration
            : settlingDuration - restDuration * globals.timeScale;
    ...
    if (!disabledY) {
      ...
      springY.velocity =
        disabledX && hasReleaseSpring ? (distanceY ? (ds * directionY) / abs(distanceY) : 0) : pv;
...
const keepTime = (constructor) => {
  ...
  return (...args) => {
    ...
    if (!isUnd(currentIterationProgress)) {
      ...
      /** @type {Tickable} */ (tracked).iterationProgress = (
        alternate ? (!(currentIteration % 2) ? reversed : !reversed) : reversed
      )
...
const parseScrollObserverFunctionParameter = (value, scroller) =>
  value && isFnc(value) ? /** @type {Function} */ (value)(scroller) : /** @type {T} */ (value);
...
class ScrollContainer {
  ...
  updateBounds() {
    ...
    const $el = this.element;
    this.scrollWidth =
      $el.scrollWidth + parseFloat(style.marginLeft) + parseFloat(style.marginRight);
    this.scrollHeight =
      $el.scrollHeight + parseFloat(style.marginTop) + parseFloat(style.marginBottom);
    this.updateWindowBounds();
    let width, height;
    if (this.useWin) {
      width = this.winWidth;
...
class ScrollObserver {
  ...
  constructor(parameters = {}) {
    ...
    const syncMethods = isMethods
      ? /** @type {String} */ (syncMode).split(" ").map((/** @type {String} */ m) => () => {
  ...
  refresh() {
    ...
    this.horizontal =
      setValue(parseScrollObserverFunctionParameter(params.axis, this), "y") === "x";
    ...
    if (shouldSeek) {
      ...
      if (syncSmooth && isNum(syncSmooth)) {
        if (/** @type {Number} */ (syncSmooth) < 1) {
          ...
          p = round$1(
            lerp$1(lp, p, lerp$1(0.01, 0.2, /** @type {Number} */ (syncSmooth))) + snap,
            6,
          );
        }
      } else if (syncEase) {
...
const validIndividualTransforms = /*#__PURE__*/ (() => [
  ...transformsShorthands,
  ...validTransforms.filter((t) => ["X", "Y", "Z"].some((axis) => t.endsWith(axis))),
])();
...
const normalizeTweenValue = (propName, value, $el, i, parsedTargets) => {
  ...
  let v = isStr(value)
    ? value
    : getFunctionValue(/** @type {any} */ (value), $el, i, parsedTargets, null, null);
  if (!isNum(v)) return v;
...
const parseIndividualTweenValue = ($el, propName, from, to, i, parsedTargets) => {
  ...
  const computedTo = !isUnd(to)
    ? normalizeTweenValue(propName, to, $el, i, parsedTargets)
  ...
  if (!isUnd(from)) {
    ...
    tweenValue = isArr(to)
      ? to.map((/** @type {any} */ v) => normalizeTweenValue(propName, v, $el, i, parsedTargets))
      : computedTo;
  }
  return tweenValue;
};
...
class WAAPIAnimation {
  ...
  constructor(targets, params) {
    ...
    this.onComplete =
      params.onComplete ||
      /** @type {Callback<WAAPIAnimation>} */ (
        /** @type {unknown} */ (globals.defaults.onComplete)
      );
    /** @type {Number} */
    this.duration = 0;
    /** @type {Boolean} */
    ...
    parsedTargets.forEach(($el, i) => {
      ...
      const hasIndividualTransforms = validIndividualTransforms.some((t) =>
      ...
      const keyEasing =
        isFnc(easeFunctionResult) || isStr(easeFunctionResult) ? easeFunctionResult : easeToParse;
      ...
      const duration =
        (spring
          ? /** @type {Spring} */ (spring).settlingDuration
          : getFunctionValue(
      ...
      for (let name in params) {
        ...
        if (isObj(propertyValue)) {
          ...
          tweenParams.duration =
            (tweenOptionsSpring
              ? /** @type {Spring} */ (tweenOptionsSpring).settlingDuration
              : getFunctionValue(
          ...
          parsedPropertyValue = isArr(propertyValue)
            ? propertyValue.map((/** @type {any} */ v) =>
                normalizeTweenValue(name, v, $el, i, parsedTargets),
              )
  ...
  forEach(callback) {
    try {
      const cb = isStr(callback)
        ? (/** @type {globalThis.Animation} */ a) => a[callback]()
        : callback;
  ...
  get currentTime() {
    ...
    return this.completed
      ? this.duration
      : controlAnimation
        ? +controlAnimation.currentTime * (timeScale === 1 ? 1 : timeScale)
        : 0;
  }
  ...
  cancel() {
    ...
    requestAnimationFrame(() => {
      this.targets.forEach(($el) => {
        // Needed to avoid unecessary inline transorms
...
const recordNodeState = (node, $measure, computedStyle, skipMeasurements) => {
  ...
  node.measuredIsRemoved =
    node.measuredHasDisplayNone || node.measuredHasVisibilityHidden || parentNotRendered;
  ...
  while (
    s &&
    (s.nodeType === Node.COMMENT_NODE || (s.nodeType === Node.TEXT_NODE && !s.textContent.trim()))
  )
    s = s.previousSibling;
  ...
  if (node.hasTransform && !skipMeasurements) {
    ...
    if ($measure === $el) {
      ...
      if (!transitionMuteStore.get($measure))
        node.measuredInlineTransition = muteElementTransition($measure);
  ...
  for (let name in properties) {
    const computedProp =
      name === "transform"
        ? computedTransforms
        : computedStyle[name] ||
          (computedStyle.getPropertyValue && computedStyle.getPropertyValue(name));
  ...
  registerElement($el, parentNode) {
    ...
    while (stack.length) {
      ...
      if (node && node.$el !== $current) {
        ...
        const shouldReassignNode =
          !nodeInsideRoot && (isInsideRoot || (!isInsideRoot && !measuredVisible && isVisible));
      ...
      node.hasVisibilitySwap =
        (hasVisibilityHidden && !node.measuredHasVisibilityHidden) ||
...
class AutoLayout {
  ...
  animate(params = {}) {
    ...
    const swapAtParams = mergeObjects(
      mergeObjects(params.swapAt || {}, this.swapAtParams),
      animationTimings,
    );
    const enterFromParams = mergeObjects(
      mergeObjects(params.enterFrom || {}, this.enterFromParams),
      animationTimings,
    );
    const leaveToParams = mergeObjects(
      mergeObjects(params.leaveTo || {}, this.leaveToParams),
      animationTimings,
    );
    ...
    newState.forEachRootNode((node) => {
      ...
      if (
        !oldStateNode.measuredIsRemoved &&
        !isRemovedNow &&
        !hasNoOldState &&
        (parentChanged || elementChanged)
      ) {
        ...
        const borderLeft = oldParent
          ? oldParent.properties.clientLeft
          : newParent.properties.clientLeft;
        const borderTop = oldParent
          ? oldParent.properties.clientTop
          : newParent.properties.clientTop;
      ...
      const topLevelAdded =
        !isRemovedNow && (wasRemovedBefore || wasPendingRemoval) && !parentAdded;
    ...
    newState.forEachRootNode((node) => {
      ...
      for (let prop in nodeProperties) {
        nodeProperties[prop] = getFunctionValue(
          nodeProperties[prop],
          $el,
          node.index,
          node.targets,
        ...
        oldStateNodeProperties[prop] = getFunctionValue(
          oldStateNodeProperties[prop],
          $el,
          oldStateNode.index,
          oldStateNode.targets,
          null,
          null,
        );
      }
      ...
      const widthChanged =
        Math.abs(nodeProperties.width - oldStateNodeProperties.width) > sizeTolerance;
      const heightChanged =
        Math.abs(nodeProperties.height - oldStateNodeProperties.height) > sizeTolerance;
      ...
      if (
        node.isTarget &&
        ((!node.measuredIsRemoved && oldStateNode.measuredIsVisible) ||
    ...
    if (targets.length) {
      ...
      for (let i = 0, l = animated.length; i < l; i++) {
        ...
        /** @type {AnimationParams} */
        const animatedProps = { composition: "none" };
        ...
        if (!newNode.hasTransform && !newNode.isInlined) {
          animatedProps.translate = [
            `${oldNodeState.x}px ${oldNodeState.y}px`,
            `${newNodeState.x}px ${newNodeState.y}px`,
          ];
          nodeHasChanged = true;
        }
          ...
          for (let prop in swapAtProps) {
            if (prop !== "transform") {
              animatedSwapParams[prop] = [
                {
                  from: (/** @type {HTMLElement} */ $el) => oldState.getComputedValue($el, prop),
                  to: swapAtProps[prop],
                },
                {
                  from: swapAtProps[prop],
                  to: (/** @type {HTMLElement} */ $el) => newState.getComputedValue($el, prop),
                  ease: inverseEased,
                },
              ];
            }
    ...
    if (transformedLength) {
      ...
      for (let i = 0; i < transformedLength; i++) {
        ...
        if (animatedSwap.includes($el)) {
          ...
          node.duration = getFunctionValue(
            swapAtParams.duration,
            $el,
            node.index,
            node.targets,
            null,
      ...
      this.transformAnimation = waapi.animate(transformed, {
        ...
        transform: (/** @type {HTMLElement} */ $el) => {
          ...
          return [
            oldValue,
            getFunctionValue(swapAtProps.transform, $el, node.index, node.targets, null, null),
            newValue,
          ];
        },
        autoplay: false,
 ...
 */
const roundPad = /** @type {typeof numberUtils.roundPad & ChainedRoundPad} */ (
 ...
 * @return {ChainableUtil}
 */
const padStart = /** @type {typeof numberUtils.padStart & ChainedPadStart} */ (
 ...
 * @return {ChainableUtil}
 */
const padEnd = /** @type {typeof numberUtils.padEnd & ChainedPadEnd} */ (
 ...
 * @return {ChainableUtil}
 */
const wrap = /** @type {typeof numberUtils.wrap & ChainedWrap} */ (
 ...
 * @return {ChainableUtil}
 */
const mapRange = /** @type {typeof numberUtils.mapRange & ChainedMapRange} */ (
 ...
 * @return {ChainableUtil}
 */
const degToRad = /** @type {typeof numberUtils.degToRad & ChainedDegToRad} */ (
 ...
 * @return {ChainableUtil}
 */
const radToDeg = /** @type {typeof numberUtils.radToDeg & ChainedRadToDeg} */ (
 ...
 * @return {ChainableUtil}
 */
const snap = /** @type {typeof numberUtils.snap & ChainedSnap} */ (
 ...
 * @return {ChainableUtil}
 */
const clamp = /** @type {typeof numberUtils.clamp & ChainedClamp} */ (
 ...
 * @return {ChainableUtil}
 */
const round = /** @type {typeof numberUtils.round & ChainedRound} */ (
 ...
 * @return {ChainableUtil}
 */
const lerp = /** @type {typeof numberUtils.lerp & ChainedLerp} */ (
 ...
 * @return {ChainableUtil}
 */
const damp = /** @type {typeof numberUtils.damp & ChainedDamp} */ (
...
const createSeededRandom = (seed, seededMin = 0, seededMax = 1, seededDecimalLength = 0) => {
  ...
  return (min = seededMin, max = seededMax, decimalLength = seededDecimalLength) => {
    ...
    const m = 10 ** decimalLength;
    return (
      Math.floor(((((t ^ (t >>> 14)) >>> 0) / 4294967296) * (max - min + 1 / m) + min) * m) / m
    );
  };
};
...
const stagger = (val, params = {}) => {
  ...
  return (target, i, t, _, tl) => {
    ...
    const customIndex = !isUnd(useProp)
      ? isFnc(useProp)
        ? useProp(registeredTarget, i, total)
        : getOriginalAnimatableValue(registeredTarget, useProp)
      : false;
    const staggerIndex =
      isNum(customIndex) || (isStr(customIndex) && isNum(+customIndex)) ? +customIndex : i;
    ...
    if (isUnd(cachedOffset)) {
      cachedOffset = tl
        ? parseTimelinePosition(tl, isUnd(params.start) ? tl.iterationDuration : start)
        : /** @type {Number} */ (start);
    }
    /** @type {String|Number} */
...
const getPath = (path) => {
  ...
  if (!$parsedSvg || !isSvg($parsedSvg))
    return console.warn(`${path} is not a valid SVGGeometryElement`);
  return $parsedSvg;
};
        ...
        return (...args) => {
          if (args[0] === "draw") {
            ...
            const d2 =
              ...
              ((v1 === 0 && v2 === 1) || (v1 === 1 && v2 === 0) ? 0 : 10 * scaleFactor) -
              d1;
  ...
  ($path1, index, total, prevTween) => {
    ...
    if (!precision) {
      ...
      for (let i = 0; i < maxPoints; i++) {
        ...
        const pointOnPath1 = /** @type {SVGGeometryElement} */ ($path1).getPointAtLength(
          length1 * t,
        );
...
const getAllTopLevelElements = ($el, type) => [
  .../** @type {*} */ ($el.querySelectorAll(`[data-${type}]:not([data-${type}] [data-${type}])`)),
];
...
const processHTMLTemplate = (
  ...
  const $split =
    /** @type {HTMLElement} */ ($content.querySelector(`[data-${type}]`)) || $highestParent;
...
class TextSplitter {
  ...
  constructor(target, parameters = {}) {
    ...
    const $target = /** @type {HTMLElement} */ (
      (target = isArr(target) ? target[0] : target) && /** @type {Node} */ (target).nodeType
        ? target
    ...
    this.lineTemplate = isObj(lineParams)
      ? generateTemplate(lineType, /** @type {SplitTemplateParams} */ (lineParams))
      : lineParams;
    /** @type {String|false|SplitFunctionValue} */
    this.wordTemplate =
      isObj(wordParams) || this.linesOnly
        ? generateTemplate(wordType, /** @type {SplitTemplateParams} */ (wordParams))
        : wordParams;
    /** @type {String|false|SplitFunctionValue} */
    this.charTemplate = isObj(charParams)
      ? generateTemplate(charType, /** @type {SplitTemplateParams} */ (charParams))
      : charParams;
    this.$target = $target;
        ...
        for (let i = 0, l = tempWords.length; i < l; i++) {
          ...
          if (!word.trim()) {
            ...
            const $charsFragment = charTemplate
              ...
              : doc.createTextNode(hasWordFollowingSpace ? word + "\xa0" : word);
            if (charTemplate) {
...
const scrambleText = (params = {}) => {
  ...
  const cursorChars =
    cursorParam === true
      ? "_"
      : typeof cursorParam === "number"
        ? String.fromCharCode(cursorParam)
  ...
  return (target, index, targets, prevTween) => {
    const rawChars =
      typeof charsParam === "function"
        ? charsParam(target, index, targets)
        : charsParam || "a-zA-Z0-9!%#_";
    const characters = expandCharRanges(charSets[rawChars] || rawChars);
    const totalChars = characters.length - 1;
    const duration =
      typeof durationParam === "function" ? durationParam(target, index, targets) : durationParam;
    const revealDelay =
      typeof revealDelayParam === "function"
        ? revealDelayParam(target, index, targets)
        : revealDelayParam || 0;
    const delay =
      typeof delayParam === "function" ? delayParam(target, index, targets) : delayParam || 0;
    ...
    const overrideChars =
      override === true
        ...
        : typeof override === "string" && override.length > 0
          ? expandCharRanges(
              charSets[/** @type {String} */ (override)] || /** @type {String} */ (override),
            )
          : null;
    ...
    const computedDuration =
      round$1((animDuration + revealDelay) / globals.timeScale, 0) * globals.timeScale;
    ...
    const resolvedFrom =
      fromParam === undefined || fromParam === "auto"
    ...
    if (resolvedFrom === "random") {
      ...
      const ref =
        resolvedFrom === "right"
          ? (override === "" || !startLength ? animLength : startLength) - 1
          : resolvedFrom === "center"
            ? ((override === "" || !startLength ? animLength : startLength) - 1) / 2
    ...
    const overrideCache = overrideChars
      ? overrideChars === characters
        ? charCache
        : new Array(animLength)
      : null;
    if (overrideCache && overrideCache !== charCache) {
      for (let c = 0; c < animLength; c++) {
        overrideCache[c] =
          overrideChar || /** @type {String} */ (overrideChars)[rng(0, overrideChars.length - 1)];
      }
    }
    ...
    if (!prevTween) {
      if (override === "") {
        ...
        for (let c = 0; c < startLength; c++) {
          fillStartText +=
            startingText[c] === " " ? " " : /** @type {Array<String>} */ (overrideCache)[c];
        }
      }
    }
            ...
            if (hasOverride && c < startLength) {
              if (hasOverrideChars) {
                if (startingText[c] === " ") {
                  ...
                  if (refreshChars)
                    /** @type {Array<String>} */ (overrideCache)[c] =
                      overrideChar ||
                      /** @type {String} */ (overrideChars)[rng(0, totalOverrideChars)];
